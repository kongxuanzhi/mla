...
int main(int argv, char* argc[]) {
	...
	if (argv < 6) {
		std::cout << "Usage: ./test_gbdt [training_file] [max_depth] [max_node_path] [tree_num] [min_sample_cnt] [learning rate]" << std::endl;
		return 0;
	}
	...
	// train the regreTree 
	mla::gbdt::GradientBoostingRegressionTree *g = new mla::gbdt::GradientBoostingRegressionTree(treeNum, maxDepth, maxNodePath, minSampleCnt, learningRate);
